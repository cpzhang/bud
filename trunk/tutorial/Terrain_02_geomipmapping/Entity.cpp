...
bool Entity::intersect( const Ray& r, Real& p )
{
	for (size_t i = 0; i != mSubEntities.size(); ++i)
	{
		bool b = mSubEntities[i]->intersect(r, p);
		if (b)
		{
			return b;
		}
...
void Entity::geomipmapping( u32 lod )
{
	for (size_t i = 0; i != mSubEntities.size(); ++i)
	{
		mSubEntities[i]->geomipmapping(lod);
	}
}
