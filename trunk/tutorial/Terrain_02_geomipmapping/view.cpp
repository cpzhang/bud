...
LRESULT CView::OnSize( UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& bHandled )
{
	...
	if(!RenderEngineImp::isNull())
		RenderEngineImp::getInstancePtr()->getCanvas()->onSize(LOWORD(lParam), HIWORD(lParam));
	return 0;
} 
...
LRESULT CView::OnKeyUp( UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled )
{
	...
	bHandled = false;
	if(!RenderEngineImp::isNull())
		RenderEngineImp::getInstancePtr()->getCanvas()->OnKeyUp(uMsg, wParam, lParam, bHandled);
	return 0;
}
