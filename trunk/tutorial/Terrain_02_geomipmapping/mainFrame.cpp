...
LRESULT CMainFrame::OnFileOpen(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	...
	CFileDialog fd(true, TEXT("entity"), NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, _T("Plato Documents (*.group;*.entity;*.mesh;*.mz;*.subentity)\0*.group;*.entity;*.mesh;*.mz;*.subentity\0All Files (*.*)\0*.*\0"));
	if (fd.DoModal() == IDOK)
	{
...
void CMainFrame::_createCanvas()
{
	...
	//
	mInfomationDlg.Create(m_vSplitPropertySheet);
	m_vSplitPropertySheet.SetSplitterPane(SPLIT_PANE_RIGHT, mInfomationDlg);

	//
	m_vSplitPropertySheet.m_cxyMin = 900;
	UpdateLayout();
	m_vSplitPropertySheet.SetSplitterPos(900);
}
