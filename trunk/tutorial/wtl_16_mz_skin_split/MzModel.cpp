...
void MzModel::decode(const std::string& fileName)
{
	...
	for (int i = 0; i < Euclid::MZ::getSubMeshNumber(); ++i)
	{
		...
		m.saveBoneMapping(Buddha::FileSystem::getInstancePtr()->getDataDirectory() + "mesh/" 
		...
		//
		g->setSkin(skinPath + "Ready01.skin");
		//
		g->setBoneMapping(Buddha::FileSystem::getInstancePtr()->getDataDirectory() + "mesh/" 
			+ Euclid::MZ::getSubMeshName(i) + ".boneMapping");
	}
}
void MzModel::render()
