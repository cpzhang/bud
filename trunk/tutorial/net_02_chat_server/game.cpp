...
bool WaitingForYou::createModules()
{
	...
	//_listenSock.init(8000);
	_listenTask = new Shannon::ListenTask;
	_listenThread = Buddha::Thread::create(_listenTask, 1024*4*4);
	_listenTask->init( 8000, 1048576 );
//	_listenTask->requireExit();
	_listenThread->start();
	//
