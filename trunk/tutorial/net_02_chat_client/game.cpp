...
bool WaitingForYou::createModules()
{
	...
	_dumpCB.onDump();
	//
	_tcpSocket.connect(Shannon::Address("localhost", 8000));
...
void WaitingForYou::send()
{
	if (_tcpSocket.connected())
	{
		std::string content("hello I Am the Client");
		u32 len = content.size();
		_tcpSocket.send((u8*)content.c_str(), len);
		MessageBox(NULL, (char*)content.c_str(), "client send", MB_OK);
	}
}

void WaitingForYou::recieve()
{
	if (_tcpSocket.connected())
	{
		char content[512];
		...
		u32 len = 512;
		if (Shannon::Sock::eSockResult_Ok == _tcpSocket.receive((u8*)content, len))
		{
			MessageBox(NULL, content, "client recieve", MB_OK);
		}
	}
}
