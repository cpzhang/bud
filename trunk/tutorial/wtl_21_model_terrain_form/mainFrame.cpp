...
void CMainFrame::_createVerticalSplitter()
{
	...
	//
//	m_lPane.Create(m_vSplit, "information");
...
void CMainFrame::_createCanvas()
{
	...
	m_view.Create(m_vSplit);
	m_vSplit.SetSplitterPane(SPLIT_PANE_RIGHT, m_view);
}

void CMainFrame::_createHoricalSplitter()
{
	//m_hSplit.Create(m_lPane, rcDefault);
	m_hSplit.Create(m_vSplit, rcDefault);
	//m_lPane.SetClient(m_hSplit);

	m_vSplit.SetSplitterPane(SPLIT_PANE_LEFT, m_hSplit);
	// set the vertical splitter parameters
	m_hSplit.m_cxyMin = 250; // minimum size
	UpdateLayout();
	m_hSplit.SetSplitterPos(500);
	// create the left container
	//m_uPane.Create(m_hSplit, "data");
	//m_hSplit.SetSplitterPane(SPLIT_PANE_TOP, m_uPane);
...
{
#if 0
	m_hSplitAniSpeed.Create(m_hSplit, rcDefault);
	m_hSplit.SetSplitterPane(SPLIT_PANE_BOTTOM, m_hSplitAniSpeed);

	m_hSplitAniSpeed.m_cxyMin = 150; // minimum size
	UpdateLayout();
	m_hSplitAniSpeed.SetSplitterPos(200);

	m_animationBox.Create(m_hSplitAniSpeed, rcDefault, NULL, WS_CHILD|WS_VISIBLE
		|CBS_DROPDOWNLIST
		|CBS_OWNERDRAWFIXED
	...
	//m_dPane.SetClient(m_animationBox);
	m_hSplitAniSpeed.SetSplitterPane(SPLIT_PANE_TOP, m_animationBox);

	//
	// 	m_animationBox.SetDroppedWidth(20);
	// 	m_animationBox.ShowDropDown(false);
	// 	m_animationBox.ModifyStyle(CBS_SORT,0);
	//
	m_speedBar.Create(m_hSplitAniSpeed, rcDefault, NULL, TBS_AUTOTICKS|WS_VISIBLE|WS_CHILD);
	m_speedBar.SetTic(5);
	m_speedBar.SetTicFreq(5);
	m_speedBar.SetRange(0, 100, TRUE);
	m_speedBar.SetPos(3);
	m_hSplitAniSpeed.SetSplitterPane(SPLIT_PANE_BOTTOM, m_speedBar);
#endif
	//
	//HWND hDlg;
	//::GetDlgItem(hDlg, IDD_FORMVIEW_ANIMATION);
	//m_aniDlg.DoModal();
	
	//m_aniDlg.Attach(m_hSplit);
}
