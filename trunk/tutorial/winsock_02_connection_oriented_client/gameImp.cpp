...
void GameImp::_initAddress()
{
	...
	if (0 == connect(ListeningSocket, (SOCKADDR *) &address, sizeof(address)))
	{
		_ss<<"connected"<<std::endl;
		std::string words(64, 0);
		size_t len = 64;
		::recv(ListeningSocket, (char*)words.c_str(), len, 0);
		_ss<<"words "<<words<<std::endl;
		words = "This is a client";
		::send(ListeningSocket, words.c_str(), words.size(), 0);
	}
