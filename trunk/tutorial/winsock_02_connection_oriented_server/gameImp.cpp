...
void GameImp::_initAddress()
{
	...
	{
		_ss<<"accepted:       "<<NewConnection<<std::endl;
		std::string words("this is the server!");
		int sendLen = ::send(NewConnection, words.c_str(), words.size(), 0);
		words.clear();
		words.resize(64, 0);
		size_t len = 64;
		::recv(NewConnection, (char*)words.c_str(), len, 0);
