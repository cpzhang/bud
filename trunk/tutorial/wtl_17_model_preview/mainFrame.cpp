	...
	//
	{
		...
		//
		{
			...
			// create the left container
 			m_rPane.Create(m_vSplit.m_hWnd, "canvas");
		...
		{
			// create the left container
			m_lPane.Create(m_vSplit, "resource");

			// add container to left pane (0) of vertical splitter
			m_vSplit.SetSplitterPane(SPLIT_PANE_LEFT, m_lPane);
			//
			m_tvCtrl.Create(m_lPane, rcDefault, NULL, 
						...
						do
						{
							...
							if(name != "." && name != ".." )
							{
								...
								else
								{
									...
									if (name == "mz" || name == "mesh" || name == "entity" || name == "group")
									{
