...
void GameImp::_initAddress()
{
	...
	address.sin_addr.s_addr = inet_addr("172.16.10.16");

	//
	{
		std::string words("this is a connectionless client!");
		int sendLen = ::sendto(SendingSocket , words.c_str(), words.size(), 0, (sockaddr*)&address, sizeof(address));
		words.clear();
		words.resize(64, 0);
		size_t len = 64;
	}
	{
		...
		::recvfrom(SendingSocket, (char*)words.c_str(), len, 0, (sockaddr*)&address, &SenderAddrSize);
		_ss<<"words "<<words<<std::endl;
	}

	// At this point you can do two things with these sockets. Wait
	// for more connections by calling accept again on SendingSocket 
	...
	// on SendingSocket , you should close the sockets using the
	// closesocket API
