...
bool Canvas::_create()
{
	...
	Euclid::sViewPort* vp = RenderEngineImp::getInstancePtr()->getViewPort();
	g_flashPlayer = g_flashDX->CreatePlayer(512, 512);
	//
	//---------------------------------------------------------------------
	// Load and play movie
	//---------------------------------------------------------------------
	//const wchar_t* movie_path = L"f:/plato/data/flash/VT.swf";
	const char* movie_path = "C:/VT.swf";
	g_flashPlayer->LoadMovie(movie_path);

	g_flashPlayer->SetTransparencyMode(IFlashDXPlayer::TMODE_FULL_ALPHA);
...
void Canvas::update()
{
	...
	if (g_flashPlayer)
	{
		...
		{
			_texTerrain->update(g_flashPlayer);
			_fxTerrain->setTexture("g_MeshTexture", _texTerrain);
		}
	}
}
