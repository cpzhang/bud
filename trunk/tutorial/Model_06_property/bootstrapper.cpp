...
class CPropertyDlg : public CDialogImpl<CPropertyDlg>, public IObserver
{
public:
	IObserver_Derived
	...
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		mTab.InsertItem(eTab_Geometry, TEXT("Geometry"));
		mTab.InsertItem(eTab_Material, TEXT("Material"));
		mTab.InsertItem(eTab_SubEntity, TEXT("SubEntity"));
		mTab.InsertItem(eTab_Entity, TEXT("Entity"));
		mTabLabels[eTab_Geometry] = mGeometryDlg.Create(mTab);
		mTabLabels[eTab_Material] = mMaterialDlg.Create(mTab);
		mTabLabels[eTab_SubEntity] = mSubEntityDlg.Create(mTab);
		mTabLabels[eTab_Entity] = mEntityDlg.Create(mTab);
		//
		setTabLabel(0);
		//
		CenterWindow();
	...
	void setTabLabel(int i)
	{
		for (size_t m = 0; m != eTab_Size; ++m)
		{
			if (m == i)
			{
				mTabLabels[m].ShowWindow(SW_SHOW);
			}
			else
			{
				mTabLabels[m].ShowWindow(SW_HIDE);
			}
		}
	}
...
private:
	...
	CSubEntityDlg mSubEntityDlg;
	CEntityDlg mEntityDlg;
	CWindow mTabLabels[eTab_Size];
};
void CPropertyDlg::update(IPrimitive* p)
{
}
