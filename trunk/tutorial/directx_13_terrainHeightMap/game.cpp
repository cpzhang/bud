...
bool WaitingForYou::initGeometry()
{
	...
	if (f.good())
	{
		...
		//
		{
			_vb = _modules->getRenderEngine()->getBufferManager()->createVertexBuffer(length * sizeof(Euclid::sPosition), Euclid::eUsage_WriteOnly, Euclid::ePool_Default);
			void* data = _vb->lock(0, 0, Euclid::eLock_Null);
			Euclid::sPosition* dataPos = (Euclid::sPosition*)data;
			...
			for (int j = 0; j != width; ++j)
			{
				...
				p.position.y = (buffer[i + j * width] - 128) * 0.25f;
				memcpy(dataPos, &p, sizeof(Euclid::sPosition));
				++dataPos;
			}
			_vb->unLock();
			//
	...
	f.close();
	//
	_fx = _modules->getRenderEngine()->getEffectManager()->createEffectFromFile("shader\\PositionTransformation.fx");
	if (NULL == _fx)
	{
		return false;
	}
...
#define Show_Terrain
#ifdef Show_Terrain
		_modules->getRenderEngine()->getRenderSystem()->setVertexDeclaration(Euclid::eVertexDeclarationType_Position);
		...
		//
		{
			...
			{
				_fx->beginPass(i);
				//_modules->getRenderEngine()->getRenderSystem()->drawPrimitive(Euclid::ePrimitive_LineStrip, 0, 128*128-1);
