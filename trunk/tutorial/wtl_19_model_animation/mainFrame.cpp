...
void CMainFrame::_createVerticalSplitter()
{
	...
	m_vSplit.m_cxyMin = 300; // minimum size
}
...
void CMainFrame::_createHoricalSplitter()
{
	...
	// set the vertical splitter parameters
	m_hSplit.m_cxyMin = 500; // minimum size
...
{
	//
	m_tvCtrl.Create(m_uPane, rcDefault, NULL, 
...
{
	m_animationBox.Create(m_dPane, rcDefault, NULL, WS_VSCROLL|WS_CHILD|WS_VISIBLE|CBS_DROPDOWNLIST|CBS_NOINTEGRALHEIGHT|CBS_AUTOHSCROLL|CBS_DISABLENOSCROLL);
	m_dPane.SetClient(m_animationBox);
	//
	m_animationBox.SetDroppedWidth(20);
	m_animationBox.ShowDropDown(TRUE);
	m_animationBox.ModifyStyle(CBS_SORT,0);
	//
	m_speedBar.Create(m_dPane, rcDefault, NULL, TBS_AUTOTICKS|WS_VISIBLE|WS_CHILD);
	m_speedBar.SetTic(5);
