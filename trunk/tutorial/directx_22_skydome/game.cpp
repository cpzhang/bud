...
bool WaitingForYou::initGeometry()
{
	...
	{
		_fx_skybox = _modules->getRenderEngine()->getEffectManager()->createEffectFromFile("shader\\PositionTexture.fx");
	}
	{
	...
	if(0)
	{
		float step_alpha = Euler::TwoPI / segment_skydome;
		float step_beta = Euler::HalfPI / segment_skydome;
		float radius = 10;//500.0f;
		for (float beta = Euler::HalfPI; beta >= 0.0f; beta -= step_beta)
		{
			Euclid::sPositionTexture vertex;
			vertex.texcoord.y = beta / Euler::HalfPI;
			for (float alpha = Euler::TwoPI; alpha >= 0.0f; alpha -= step_alpha)
			{
				vertex.position.x = radius * Euler::Basic::Cos(beta) * Euler::Basic::Cos(alpha);
				vertex.position.y = radius * Euler::Basic::Sin(beta);
				vertex.position.z = radius * Euler::Basic::Cos(beta) * Euler::Basic::Sin(alpha);
				vertex.texcoord.x = alpha / Euler::TwoPI;
	...
	}
	{
		...
		for (int i = 0; i <= segment_skydome - 1;i += offset)
		{
			for (int j = 0; j <= segment_skydome - 1; j += offset)
			{
				...
				if (0&& j > segment_skydome - 1)
				{
					//
					_indices_skydome.push_back(baseIndex);
					_indices_skydome.push_back(baseIndex + segment_skydome * offset);
					_indices_skydome.push_back(baseIndex + offset - segment_skydome);
					//
					_indices_skydome.push_back(baseIndex + segment_skydome * offset);
					_indices_skydome.push_back(baseIndex + offset + segment_skydome * offset - segment_skydome);
					_indices_skydome.push_back(baseIndex + offset - segment_skydome);
				}
				else
				{
					...
					//
					_indices_skydome.push_back(baseIndex + segment_skydome * offset);
					_indices_skydome.push_back(baseIndex + offset + segment_skydome * offset);
					_indices_skydome.push_back(baseIndex + offset);
				}
			}
		}
	...
	{
		_texture_skydome = _modules->getRenderEngine()->getTextureManager()->createTextureFromFile("image/dome/Sky_horiz_10_4096.jpg");
	}
	return true;
}
