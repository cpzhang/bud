...
bool g_b25d = true;
Mat4 g_mtxProjection,g_mtxView45,g_mtxView;
	...
	case WM_TIMER:
		{
			...
			if(g_pNode)
			{
				...
				if(1)
				{
					...
					Vector3 lightDirection = Vector3(-1.0f,2.5f,0.0f);
					Mat4 mtxWorld = g_pRenderSystem->getWorldMatrix();
					Mat4 mtxShadow = Matrix4::IDENTITY;
				...
				g_pRenderSystem->setAlphaCheckEnabled(true);
			...
			if(g_pNode && g_bShowBoneAxis)
			{
				...
				{
					Mat4 mtxTranslation = Matrix4::IDENTITY;
					mtxTranslation.setTrans(g_pNode->getModelInstance()->getBone(i)->getTransformedPivot());
					Mat4 mtxRotation = g_pNode->getModelInstance()->getBone(i)->getFullRotation();
			...
			if(g_pNode)
			{
				...
				if(g_b25d)
				{
					sprintf(str,"A 向左旋转");
					wstr = str;
					g_pFont->render2d(0,184,0,ColorValue::Green,wstr.c_wstr());
					sprintf(str,"D 向右旋转");
					wstr = str;
					g_pFont->render2d(0,200,0,ColorValue::Green,wstr.c_wstr());
				}
			}
			g_pRenderSystem->switchTo3D();
	...
	case WM_PREVIEW:
		{
			...
			tempFileName += "maya.mz";

			g_pNode = ModelNodeCreater::create(tempFileName.c_str(),0,0);
			if(g_pNode)
			{
				g_pNode->setOrientation(g_orientation);
				g_pNode->getModelInstance()->setCurrentAnimation("Animation");
			}
			SetActiveWindow(g_hWnd);
			SetForegroundWindow(g_hWnd);
			BringWindowToTop(g_hWnd);
			SetWindowPos(g_hWnd, 0, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE | SWP_SHOWWINDOW);
		}
		break;
	case WM_KEYDOWN:
		{
			...
			case 'A':
				{
					angle -= 45;
					g_orientation.FromAngleAxis(angle, Vector3(0,1,0));
					g_pNode->setOrientation(g_orientation);
				}
				break;
			case 'D':
