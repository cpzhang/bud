...
PlatoTreeEnumProc::PlatoTreeEnumProc()
{
	_clear();
}
...
int PlatoTreeEnumProc::callback( INode *node )
{
...
void PlatoTreeEnumProc::setTime( TimeValue t )
{
	_time = t;
}

void PlatoTreeEnumProc::write()
{
	//============================================================================
	// 开始写入数据
	ChunkSet cs;
...
void PlatoTreeEnumProc::addNode( INode* n)
{
	if (n->IsRootNode())
	{
	}
...
bool PlatoTreeEnumProc::IsUnReferrenced(const sBone& b)
{
...
{
	b.unReferrenced = PlatoTreeEnumProc::IsUnReferrenced(b);
}
...
void PlatoTreeEnumProc::createTreeItem(HWND hDlg, sBone& b, HTREEITEM h)
{
...
{
	static PlatoTreeEnumProc* Plato;
	switch(message) 
	{
		case WM_INITDIALOG:
			{
				Plato = (PlatoTreeEnumProc*)lParam;
				SetWindowContextHelpId(hDlg, idh_3dsexp_export);
				CenterWindow(hDlg,GetParent(hDlg));
				SetFocus(hDlg);
				{
					//
					for (std::vector<sBone>::iterator i = Plato->_boneSet.begin(); i != Plato->_boneSet.end(); ++i)
					{
						sBone& b = *i;
						if (IsUnReferrencedAndChildren(b) && !IsUnReferrencedAndChildren(Plato->_boneSet[b.parentId]))
						{
							Plato->createTreeItem(hDlg, b, NULL);
						}
					}
				}
			}
					...
					if(((LPNMHDR)lParam)->code == NM_CLICK)
					{
						...
						if (thi.flags == TVHT_ONITEMSTATEICON)
						{
							...
							if(SendDlgItemMessage(hDlg, IDC_TREE1, TVM_GETITEM, TVGN_CARET,(LPARAM)&tvi))
							{
								...
								for (size_t k = 0; k != Plato->_boneSet.size(); ++k)
								{
									if (Plato->_boneSet[k].name == boneName)
									{
										if (isChecked)
										{
											Plato->_boneSet[k].referrenced = false;
										}
										else
										{
											Plato->_boneSet[k].referrenced = true;
										}
										break;
									}
								}
...
void PlatoTreeEnumProc::_getKeyFrame( Control* tmc )
{
...
bool PlatoTreeEnumProc::_extractMaterial( INode* node, unsigned short& matID )
{
	/////////////////////////////////////////////////////////////////
...
void PlatoTreeEnumProc::_extractMesh( INode* node )
{
...
bool PlatoTreeEnumProc::_isMaterialExtracted( Mtl* m, unsigned short& matID)
{
...
void PlatoTreeEnumProc::_extractSkin( INode* node )
{
	///////////////////////////////////////////////
	// skin
	//////////////////////////////////////////////
...
Modifier* PlatoTreeEnumProc::_getPhysiqueModifier(INode *pNode)
{
...
eType PlatoTreeEnumProc::_extractType( INode* node )
{
...
void PlatoTreeEnumProc::_extractBone( INode* node )
{
...
void PlatoTreeEnumProc::readBone()
{
...
void PlatoTreeEnumProc::writeBone()
{
	//============================================================================
	// 开始写入数据
	ChunkSet cs;
...
void PlatoTreeEnumProc::_setBoneParent()
{
	for (size_t i = 0; i != _boneSet.size(); ++i)
	{
		for (size_t k = 0; k != _boneSet.size(); ++k)
...
bool PlatoTreeEnumProc::readMesh()
{
	for(size_t i = 0; i != _nodeSet.size(); ++i)
	{
		INode* node = _nodeSet[i];
...
void PlatoTreeEnumProc::_extractParticle( INode* node )
{
	ObjectState os = node->EvalWorldState(0);
...
*/
void PlatoTreeEnumProc::_extractBiped( INode* node )
{
	//
	Interface* i = GetCOREInterface();
	if (i == NULL)
...
void PlatoTreeEnumProc::readParticle()
{
	for(size_t i = 0; i != _nodeSet.size(); ++i)
	{
		INode* node = _nodeSet[i];
...
std::string PlatoTreeEnumProc::_getParticleBone( INode* node )
{
	///////////////////////////////////////////////
	// skin
	//////////////////////////////////////////////
...
bool PlatoTreeEnumProc::_isBoneSelected()
{
	for(size_t i = 0; i != _nodeSet.size(); ++i)
	{
		INode* node = _nodeSet[i];
...
void PlatoTreeEnumProc::writeMaterial()
{
	//============================================================================
	// 开始写入数据
	ChunkSet cs;
...
bool PlatoTreeEnumProc::readMaterial()
{
	for(size_t i = 0; i != _nodeSet.size(); ++i)
	{
		INode* node = _nodeSet[i];
...
bool PlatoTreeEnumProc::_extractMaterialFX( INode* node, unsigned short& matID )
{
	Mtl* m = node->GetMtl();
	...
	IDxMaterial3* dxMtl = (IDxMaterial3*)m->GetInterface(IDXMATERIAL3_INTERFACE);
	if( NULL == dxMtl)
	{
		return false;
	}
	...
	//
	{
		MaxSDK::AssetManagement::AssetUser au = dxMtl->GetEffectFile();
		std::string fileName = au.GetFileName();
		std::string fullFileName = au.GetFullFilePath();//二者一样
	}
	
	//
	{
		int nBitmaps = dxMtl->GetNumberOfEffectBitmaps();
		std::string fileName;
		std::string name;
		for (int i = 0; i < nBitmaps; ++i)
		{
			PBBitmap* p = dxMtl->GetEffectBitmap(i);
			if (p)
			{
				fileName = p->bi.Filename();//文件名
				name = p->bi.Name();//全路径，带文件名
			}
		}
	}
	...
	//
	{
		IParameterManager* pm = dxMtl->GetCurrentParameterManager();
		if (pm)
		{
			int np = pm->GetNumberOfParams();
			char data[512];
			std::string name;
			for (int i = 0; i < np; ++i)
			{
				memset(data, 0, 512 * sizeof(char));
				name = pm->GetParamName(i);
				int s = pm->GetParamSize(i);
				int t = pm->GetParamType(i);
				pm->GetParamData(data, i);
				switch(t)
				{
				case IParameterManager::kPType_Unknown:
					{
					}break;
