...
int PlatoExporter::DoExport( const MCHAR *name,ExpInterface *ei,Interface *i, BOOL suppressPrompts/*=FALSE*/, DWORD options/*=0*/ )
{
	...
	{
		GetExportOptions()->exportMode = eExportMode_Material;
		if (_enum.readMaterial())
		{
			_enum.writeMaterial();
		}
	}
...
const std::string PlatoExporter::tName("Plato Exporter of Material");
const std::string PlatoExporter::tLongDesc("Plato 3D Max Exporter of Material");
const std::string PlatoExporter::tShortDesc("Plato Exporter of Material");
