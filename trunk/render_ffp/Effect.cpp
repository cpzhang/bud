...
namespace Euclid
{
	...
	void Effect::release(RenderableObject* o)
	{
		std::vector<RenderableObject*>::iterator it = std::find(_referrencedObjects.begin(), _referrencedObjects.end(), o);
		if (it != _referrencedObjects.end())
		{
			_referrencedObjects.erase(it);
		}

		if (_referrencedObjects.empty())
		{
			release();
		}
	}
	...
	{
		_effect->SetMatrix(name.c_str(), &D3D9Mapping::makeD3DXMatrix(*pMatrix));
	}
	...
	void Effect::onSetEffect(RenderableObject* o)
	{
		std::vector<RenderableObject*>::iterator it = std::find(_referrencedObjects.begin(), _referrencedObjects.end(), o);
		if (it == _referrencedObjects.end())
		{
			_referrencedObjects.push_back(o);
		}
	}
